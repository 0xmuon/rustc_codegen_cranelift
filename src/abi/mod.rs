...
pub(super) struct ArgValue<'tcx> {
    ...
    /// If set, the argument is a byval/byref pointer whose pointee alignment is smaller than the
    /// Rust ABI alignment. In that case the argument must be copied to a sufficiently aligned
    /// local stack slot before it can be treated as a place.
